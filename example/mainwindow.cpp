...
//    connect(eepromSize, SIGNAL(accepted()), this, SLOT(setChipType(QString));
...
//
void MainWindow::readActBt()
{
    QString set_rom_operation = "echo ./ch341eeprom -r buffer.bin -s "+QString( lbChipType->text());
    //QString set_rom_operation = "./ch341eeprom -r buffer.bin -s "+QString( lbChipType->text());
    QProcess::execute(set_rom_operation);
    QMessageBox::about(this, "Title", "READ");
    loadFile("buffer.bin");
}
void MainWindow::writeActBt()
{
    saveFile("buffer.bin");
    QString set_rom_operation = "echo ./ch341eeprom -w buffer.bin -s "+QString( lbChipType->text());
    //QString set_rom_operation = "./ch341eeprom -w buffer.bin -s "+QString( lbChipType->text());
    QProcess::execute(set_rom_operation);
    QMessageBox::about(this, "Title", "WRITE");
}
